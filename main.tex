...
\section{Summary and Significance}
Implanted BCIs record neural activity that can be used to distinguish a variety of upper limb behaviors, like arm translation, wrist rotation, and hand grasp. Existing strategies use linear decoders on these neural signals to enable high degree of freedom (DoF) of control. However, the resulting control requires a substantial daily calibration, upwards of 15 minutes for 7 DoF and of 20 minutes for 10 DoF. Moreover, the resulting control is not especially robust, sensitive to changes in environmental context and nontrivial interactions between different DoF not seen in calibration. These two challenges imply complementary needs for BCI models that are both data-efficient, to minimize user burden, yet also high-capacity, to stabilize existing behavior or to define new ones in light of new patterns of neural activity. We propose pretrained NDT3 for this purpose, aiming to provide 7-9DoF control of the arm and hand and to measure its day-over-day performance for two human BCI participants.

\textbf{Significance}: This aim proposes two advances for upper limb neuroprosthetics. First, we target multi-day calibration data scaling as a reliable strategy for continuous and convenient high DoF upper limb control. This system reliability necessary to translate upper limb neuroprosthetics from the lab towards independent device use. Second, we show data aggregation does not merely stabilize existing BCI behaviors, but also enables the scaling of BCI to new behavioral repertoires. Together, these aims contribute to the framing of a BCI model as a summary reflection of previously collected calibration data.


\section{Approach}
We aim to use an NDT3-based deep network controller to provide high quality, high DoF control that can be calibrated quickly and reliably to reach high performance day after day. We expect this to be enabled by two conceptual advances for the movement BCI field. First, to enable quick BCI use across days, we will change from daily decoder retraining to a large initial calibration followed by minimal calibration for decoder adaptation to maintain performance across days. Second, we will extend this strategy to scale to more complex behaviors, primarily by adding more DoF to control. Depending on the signal tuning of our new participant, we will at minimum target two additional dimensions of thumb control, or more ambitiously target individual finger control. This minimum is justified in the literature by the robust availability of thumb tuning in several human BCI users implanted near the hand knob~\citep{willsey2024real,shah2025pseudo}. In either case, we will enable this new control with a separate hand-focused calibration dataset. The intent is to demonstrate that a BCI system can accumulate neural-behavior relations in the weights of a deep network, rather than a disparate external collection of datasets. I elaborate on the proposed methods below.

\textbf{Rapid calibration with decoder adaptation}: Previous work for high DoF arm and hand control was achieved using a 15 minute calibration period performed daily~\citep{collinger2013high}. This long calibration is necessary for training a new decoder as the requisite time to sample sufficient behavioral diversity grows with behavior dimensionality. However, this calibration burden can be reduced, as the relationship between neural activity and behavior only partially changes across days. In speech BCIs and simpler movement BCIs, decoders trained with multiple days of data can be adapted for high performance on new days with minimal, if any, calibration~\citep{sussillo_16_future,hosman2023months,fan2023plugandplay,card2025long}. We aim to replicate this strategy with high DoF motor control, aiming for calibration datasets of up to 30 minutes in early days of use, to be maintained over time with greatly abbreviated, 3 minute calibration phases. All calibration tasks will require the participant to follow a virtual robot arm performing reaching and grasping actions.

\textbf{Scaling to higher DoF control}: We will initially target 7 DoF for robot arm and hand control. For these 7 dimensions, 3 dimensions will be for hand endpoint translation, 3 dimensions will be for wrist rotation, and 1 dimension will control whole hand grasp. This control scheme has been historically used in our lab and is readily supported by current robot arm and hand capabilities. Previous work extending hand control from whole hand grasp into 4 hand-shape basis dimensions (Wodlinger) showed that the BCI user did not achieve higher clinical gain of function relative to the 1D whole hand grasp. This may have been a limitation of the linear controllers used at the time, or of the control scheme. We will probe the viability of expanding hand control by using at least 3 degrees for the hand, namely with 2D for thumb flexion/extension and abduction/adduction and 1D for flexion/extension of the other 4 fingers as one group. Depending on our ability to discriminate the other 4 fingers, we may attempt to decode them in smaller groups or individually. We will train and evaluate this new hand control scheme with a focused calibration dataset that varies the hand DoF in isolation, with the arm and wrist in neutral postures. Finally, in preparation for combined control of the hand, wrist, and arm, we must collect a bridging dataset. It is known, for example, that finger decoding will degrade as the wrist changes postures [Alexandriya?]. We expect that minimal calibration in these new static postures will suffice to restore performance. For simplicity, we still do not plan on collecting calibration datasets where multiple DoF are varying simultaneously.

\textbf{Evaluation tasks}: To compare with prior studies, we will evaluate functional performance with timed object transfer trials and the Action Research Arm Test (ARAT), for comparison with prior results in high DoF upper limb control~\citep{collinger2013high,wodlinger_15_10d}. Both protocols assess grasping and carrying competence. To evaluate the continuous stability of system performance, we will target at least 8 experimental sessions over the course of a month. In a subset of these sessions, we will compare against the previous state of the art, 7D control with linear controllers. Due to restrictions on experiment time, comparisons against other baselines and system design choices will use offline prediction-based analyses. In addition to these functional evaluations, we will include a number of virtual brain-controlled reach-grasp-carry sequences in the same setting as the virtual calibration data. These virtual evaluations will allow us to assess more fine-grained control metrics like path efficiency and movement phase-wise performance. Hand control will be evaluated exclusively in similar virtual environments. Finally, we will assess combined arm and hand control (e.g. 9D) with the same ARAT protocol as used to evaluate 7D function. We expect to be able to operate the same decoder in different DoF settings, so control be compared within the same testing day.


\section{Results \& Remaining Work}
We have collected pilot experiments that demonstrate the efficacy of NDT3 and the proposed multiday decoding strategy for lower dimensional control, namely for 2D cursor control in three participants and for 4D grasp and carry tasks in Mujoco with one participant.
...
\textbf{2D Cursor control}: We have compared NDT3 against linear controllers for 2D cursor control with three participants, each in two separate sessions. The largest NDT3 model provides control that is quantitatively comparable with the linear baseline, though qualitatively often producing slower and steadier movement. For direct comparison, we do not perform any closed loop tuning on any decoder. Across participants and over multiple sessions, we find path efficiency and acquisition time for center-out cursor evaluations to be comparable between decoders.

In practice, cursor controllers are trained not only from open loop calibration blocks, but with additional closed loop adaptation. We use the ReFIT intention estimation strategy to enable closed loop adaptation of both linear and NDT controllers. We find that linear control reliably improves with ReFIT across participants, but NDT control does not improve as significantly. This appears to be less an issue of decoding angular accuracy but rather a difficulty with control gain, as manually increasing NDT gain can significantly improve NDT control. We speculate that NDT lags OLE under the same ReFIT protocol as it suffers more significantly from label noise introduced across the multiple calibration datasets. This result motivates studying new closed loop tuning strategies, but we note that this closed loop tuning does not majorly change performance in participatns with high signal quality (P4), so we anticipate we can run high DoF control primarily relying on open loop calibration.
